...
func reward_notebook_1(npc):
	...
	npc.start_post_puzzle_dialog()  # ← usa o npc recebido

func reward_notebook_2(npc):
	var player = get_tree().get_first_node_in_group("player")
	player.blocked = true
	get_tree().paused = false
	await get_tree().create_timer(0.8).timeout
...
func start_puzzle(puzzle_id: String, npc = null):
	...
	ui.setup(
		...
		func():
			get_tree().paused = false
			data["reward"].call(npc)  # ← passa o npc pro reward
	)
